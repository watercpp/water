...
namespace _ {
	template<typename iterator_, typename size_ = void> struct
	 size_do {
	...
	template<typename iterator_> struct
	 size_do<iterator_, types::to_void<decltype(iterator_{} - iterator_{})>> {
		static size_t do_it(iterator_ begin, iterator_ end) {
...
template<typename range_, typename result_, typename = decltype(types::make<range_ const&>().begin() == types::make<range_ const&>().end())> struct
...
namespace _ {
	...
	template<typename range_> struct
	 range_size_do<range_, types::to_void<decltype(types::make<range_ const&>().size())>> {
		static size_t do_it(range_ const& a) {
...
namespace _ {
	template<typename unsigned_, bool small_ = (static_cast<unsigned_>(-1) < static_cast<size_t>(-1))> struct
	 string_size_clip_if {
		template<typename iterator_> static bool do_it(unsigned_& to, size_t size, iterator_ begin) {
	...
	template<typename unsigned_> struct
	 string_size_clip_if<unsigned_, false> {
		template<typename iterator_> static bool do_it(unsigned_& to, size_t size, iterator_) {
			to = size;
			return false;
			}
		};
	}
template<typename unsigned_, typename iterator_> bool string_size_clip(unsigned_& to, iterator_ begin, iterator_ end) {
...
template<typename char_> inline char32_t string_compare_cast(char_ a) { return static_cast<char32_t>(a); }
