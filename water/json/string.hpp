...
template<typename iterator_> string<iterator_>
 string_from(iterator_ begin, iterator_ end) {
	return {begin, end};
	}

template<typename iterator_> string<iterator_>
 string_from(iterator_ begin, size_t size) {
	return {begin, size};
	}

template<typename a_, typename b_> bool
 operator==(string<a_> const& a, string<b_> const& b) {
	if(a.size() != b.size())
		return false;
	auto ai = a.begin();
	auto bi = b.begin();
...
template<typename iterator_, size_t size_> bool
 operator==(string<iterator_> const& a, char const (&b)[size_]) {
	return a == string_from(b, size_ - 1);
	}
template<typename iterator_, size_t size_> bool
 operator==(char const (&a)[size_], string<iterator_> const& b) {
