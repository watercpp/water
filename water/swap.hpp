...
namespace _ { namespace swap_hidden_from_argument_dependent_lookup {
	...
	template<typename a_, typename b_> void swap(a_& a, b_& b) {
		a_ s(static_cast<a_&&>(a));
		a = static_cast<a_&&>(b);
		b = static_cast<a_&&>(s);
		}

	template<typename t_, decltype(sizeof(0)) s_> void swap(t_ (&a)[s_], t_ (&b)[s_]) {
...
template<typename type_> void swap_from_swap(type_& a, type_& b) {
	swap(a, b);
	}
