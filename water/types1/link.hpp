...
#ifndef WATER_TYPES1_LINK_HPP
#define WATER_TYPES1_LINK_HPP
...
This was written in 2005 or earlier, it uses only C++ 1998. Back then types1::list was implemented
with the preprocessor. That felt a bit "brute force" and it had a fixed maximum size of 100. This
was different.

Today types1::list has a much simpler implementation and might be more useful.
...
{
    typedef next_ next;
    typedef types1::collection_tag<link_kind<link<result_, next_> > > collection_tag;
};
...
struct link_at { // this is out of bounds
    typedef types1::collection_tag<link_at_kind<link0_, offset_, void> > collection_tag;
};
...
struct link_at<l0_, o_, l_<a_, n_>, 0, 0> : l_<a_, n_> { // this is not out of bounds
    typedef types1::collection_tag<link_at_kind<l0_, o_, l_<a_, n_> > > collection_tag;
};
