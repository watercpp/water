...
#ifndef WATER_TYPES1_ASSERT_HPP
#define WATER_TYPES1_ASSERT_HPP
#include <water/types1/to_bool.hpp>
namespace water { namespace types1 {

/*

type_assert<type_>::assert generates a compile time error when types1::to_bool<type_>::result is false.
...
Example: a function requires it's template argument to be a built in integer type:
    ...
    type_ function(type_ a) {
        static_cast<void>(types1::type_assert<types1::is_int<type_>>::assert);
        return a;
    }

If function(1.23) is called (with a double) the compiler error could be:
"assert is not a member of types1::type_assert<types1::is_int<double>>"
