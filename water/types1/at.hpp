...
#ifndef WATER_TYPES1_AT_HPP
#define WATER_TYPES1_AT_HPP
...
- at<c_, 123> is a collection positioned at types1::offset<c_>::result + 123
- at<at<c_, 3>, -2> is like at<c_, 1>
- if at<c_, 123> is inside the collection
  - types1::is_result<at<c_, 123> > is true
  - at<c_, 123>::result is the element
- if at<c_, 123> is outside the collection
  - types1::is_result<at<c_, 123> > is false

it must be possible to move outside the collection, and then move back inside it.
- types1::at and types1::is_result must work outside the collection

if c_ has at least 5 items, can contain types, and types1::insert can be used with it, a and b in the
