...
#ifndef WATER_TYPES1_NEXT_HPP
#define WATER_TYPES1_NEXT_HPP
...
Move to the next position in a collection
 - next<c_> is the same position in the collection as types1::at<c_, 1>
 - if next<c_> is inside the collection:
   - next<next<c_> > is valid
   - next<c_>::result is the element
   - types1::is_result<next<c_> > is true
 - if next<c_> is outside the collection:
   - types1::is_result<next<c_> > is false
