...
#ifndef WATER_TYPES1_IS_NO_TO_ADD_ARRAY_HPP
#define WATER_TYPES1_IS_NO_TO_ADD_ARRAY_HPP
...
namespace _ {
    ...
    template<typename>
    struct do_is_array : false_result {};

    template<typename a_, size_t b_>
    struct do_is_array<a_[b_]> : true_result {};
    ...
    struct do_no_array : type_plain<a_> {};

    template<typename a_, size_t b_>
    struct do_no_array<a_[b_]> : type_plain<a_> {};
    ...
    template<typename a_, size_t b_>
    struct do_to_array : type_plain<a_[b_]> {};

    template<typename a_, size_t b_>
    struct do_to_array<a_&, b_> : type_plain<a_(&)[b_]> {};

    template<typename a_, size_t b_>
    struct do_to_array<a_&&, b_> : type_plain<a_(&&)[b_]> {};

    template<typename a_, size_t b_, size_t c_>
    struct do_to_array<a_[b_], c_> : type_plain<a_[c_]> {};
    ...
    template<typename a_, size_t b_>
    struct do_add_array : type_plain<a_[b_]> {};

    template<typename a_, size_t b_>
    struct do_add_array<a_&, b_> : type_plain<a_(&)[b_]> {};
    ...
    template<typename>
    struct do_array_size : integer<size_t, 0> {};

    template<typename a_, size_t b_>
    struct do_array_size<a_[b_]> : integer<size_t, b_> {};
...
// result number of elements in array (types1::size_t),
// 0 if not types1::is_array<type_>::result
