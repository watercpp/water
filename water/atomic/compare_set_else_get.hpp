...
#if !defined(WATER_ATOMIC_COMPARE_SET_ELSE_GET_HPP) || defined(WATER_ATOMIC_INLINE)
#ifndef WATER_ATOMIC_INLINE
#define WATER_ATOMIC_COMPARE_SET_ELSE_GET_HPP
...
template<barrier_t barrier_, typename type_> if_type<compare_set_else_get_exists<type_, barrier_>, bool>
 compare_set_else_get(type_& atomic, value<type_> compare, value<type_> set, value<type_>& get) {
	...
	//
	// This is is made using atomic::get_compare_set, if it exists in the the underlying implementation.
	// If it does not, atomic::compare_set and atomic::get is used like this:
	//
	//   if(atomic::compare_set<barrier_>(atomic, compare, set))
	//       return true;
	//   get = atomic::get<atomic::none>(atomic);
	//   return false;
	//
	// This means it can be 2 atomic operations, although atomic::get is used with no barrier. 
	// It is possible that get == compare even after this function returns false, because of the 2 atomic operations.
	// atomic::compare_set_else_non_atomic_get can sometimes be used instead of this, and it could be more efficent.
	//
	// Example:
	//
	//   void atomic_add1(atomic::int_t& a) {
	//       atomic::int_t b = atomic::get<atomic::none>(a);
	//       while(!atomic::compare_set_else_get(a, b, b + 1, b));
	//       }
	//
