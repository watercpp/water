...
#if !defined(WATER_ATOMIC_BITS_HPP) || defined(WATER_ATOMIC_INLINE)
#ifndef WATER_ATOMIC_INLINE
#define WATER_ATOMIC_BITS_HPP
#include <water/atomic/atomic_tag.hpp>
#include <water/atomic/configuration.hpp>
#include <water/atomic/constants.hpp>
#include <water/atomic/raw_cast.hpp>
namespace water { namespace atomic {
#endif

#ifdef WATER_ATOMIC_INLINE

	namespace types = water::types;
	
	using water::atomic::atomic_tag;
	using water::atomic::atomic_type_of;
	using water::atomic::value_type_of;
	using water::atomic::value;
	
	using namespace water::atomic::constants;
	
	using water::atomic::raw_cast;
...
// result true if using type_ inside water::atomic will not break c++ aliasing rules (c++ standard 3.10.15). it is true when:
