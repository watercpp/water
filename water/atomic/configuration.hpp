...
#ifndef WATER_ATOMIC_CONFIGURATION_HPP
#define WATER_ATOMIC_CONFIGURATION_HPP
#include <water/water.hpp>

// to use a new underlying implementation with water atomic
// - modify this header to know how to select it, or...
// - include the underlying before including anything else in water::atomic
//   and define WATER_ATOMIC_UNDERLYING to the fully qualified namespace of
//   the underlying

#ifdef WATER_ATOMIC_UNDERLYING
namespace water { namespace atomic {
	namespace underlying = WATER_ATOMIC_UNDERLYING;
}}
		
// atomic_ops library, you must define WATER_ATOMIC_HAS_ATOMIC_OPS
#elif defined(WATER_ATOMIC_HAS_ATOMIC_OPS)
#include <water/atomic/atomic_ops/atomic_ops.hpp>
namespace water { namespace atomic {
	namespace underlying = atomic_ops;
}}

#elif defined(WATER_ATOMIC_HAS_GCC) || (defined(WATER_COMPILER_GCC) && WATER_COMPILER_GCC >= 41) || defined(WATER_COMPILER_CLANG)
...
#include <water/atomic/interlocked/interlocked_windows.hpp>
namespace water { namespace atomic {
...
namespace water { namespace atomic { namespace underlying {
