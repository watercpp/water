...
#if !defined(WATER_ATOMIC_GET_COMPARE_SET_HPP) || defined(WATER_ATOMIC_INLINE)
#ifndef WATER_ATOMIC_INLINE
#define WATER_ATOMIC_GET_COMPARE_SET_HPP
...
namespace _ {
	...
	template<typename t_, barrier_t b_> struct
	 get_compare_set<t_, b_, has_compare_set> {
		static t_ do_it(t_* a, t_ b, t_ c) {
			// atomic::get<none> can be compare_set in a loop too
			t_ r = b;
			while(
