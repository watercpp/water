...
template<clockid> inline timespec clock() noexcept {
	timespec r{};
...
template<clockid> inline bool is_monotonic() noexcept {
	return false;
	}

#if defined(WATER_POSIX_CLOCK_MONOTONIC) && (!defined(_POSIX_MONOTONIC_CLOCK) || _POSIX_MONOTONIC_CLOCK > 0)

template<> inline bool is_monotonic<clockid::monotonic_maybe>() noexcept {
	return true;
	}

template<> inline timespec clock<clockid::monotonic_maybe>() noexcept {
	timespec r{};
...
template<clockid clockid_> class
 deadline_clock {
	private:
		timespec my;
		bool myleft = true;
