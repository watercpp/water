...
template<typename call_> void* thread(void* pointer) {
...
template<typename call_> bool run(typename call_::pointer p, thread_t& t, pthread_attr_t const* a) noexcept {
...
template<typename call_> bool run(typename call_::pointer p, join_t& j, pthread_attr_t const* a) noexcept {
	return run<call_>(p, j.thread, a);
	}

template<typename call_> bool run(typename call_::pointer p, join_t& j) noexcept {
	return run<call_>(p, j.thread, 0);
	}

template<typename call_> bool run(typename call_::pointer p, join_t& j, run_options const& o) noexcept {
	pthread_attr_t a;
	bool r = false;
	if(!pthread_attr_init(&a)) {
...
template<typename call_> bool run(typename call_::pointer p, thread_t& t, run_options const& o = {}) noexcept {
	pthread_attr_t a;
	bool r = false;
	if(!pthread_attr_init(&a)) {
...
template<typename call_> bool run(typename call_::pointer p, run_options const& o = {}) noexcept {
	thread_t t;
...
template<typename function_> bool run(function_& f, join_t& j) noexcept {
	return run<run_function<function_>>(&f, j);
	}

template<typename function_> bool run(function_& f, join_t& j, run_options const& o) noexcept {
	return run<run_function<function_>>(&f, j, o);
	}

template<typename function_> bool run(function_& f, thread_t& t, run_options const& o = {}) noexcept {
	return run<run_function<function_>>(&f, t, o);
	}

template<typename function_> bool run(function_& f, run_options const& o = {}) noexcept {
