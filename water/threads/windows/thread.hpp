...
template<typename call_> void*
 run(dword_t& id, typename call_::pointer pointer, security_attributes_t *security, size_t stack_size, dword_t creation_flags, int priority) noexcept {
...
template<typename call_> bool run(typename call_::pointer p, join_t& j, run_options const& o = run_options()) noexcept {
	thread_t id;
	return (j.handle = run<call_>(id, p, o.security(), o.stack_size(), o.flags(), o.priority_windows())) != 0;
	}

template<typename call_> bool run(typename call_::pointer p, thread_t& t, run_options const& o = run_options()) noexcept {
