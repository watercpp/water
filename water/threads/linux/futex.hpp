...
template<typename int_> if_futex<int_, int>
 futex_wait(int_& futex, unsigned value, timespec const* timeout = 0) noexcept {
	// if futex == value, wait
	// - timeout is relative
...
template<typename int_> if_futex<int_, int>
 futex_wait(int_& futex, unsigned value, double seconds) noexcept {
	timespec t{};
...
template<typename int_> if_futex<int_, int>
 futex_wake(int_& futex, unsigned count = 1) noexcept {
...
template<typename int_> if_futex<int_, int>
 futex_wake_all(int_& futex) noexcept {
	return futex_wake(futex, futex_max);
	}

template<typename int1_, typename int2_> if_futex<int1_, int>
 futex_cmp_requeue(int1_& futex, unsigned value, unsigned count, int2_& requeue_futex, unsigned requeue_count = futex_max) noexcept {
	// if futex == value
	// - wake count threads
