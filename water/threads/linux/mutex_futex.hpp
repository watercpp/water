 ...
 mutex_futex {
	...
	public:
		...
		void lock() noexcept {
			...
			if(atomic::compare_set<atomic::acquire>(my, 0, 1))
				return;
			___water_threads_statistics(add.wait(false));
			pause p = pause_wait();
			while(atomic::get_set(my, 2))
		...
		void unlock() noexcept {
			...
			if(atomic::get_set<atomic::release>(my, 0) == 2) {
				futex_wake(my);
				___water_threads_statistics(add.wake(false));
				}
			}
		futex_t& underlying() noexcept {
			return my;
			}
