...
namespace _ { namespace statistic {
	...
	template<typename unsigned_> void add(unsigned_& to, size_t add = 1) {
	...
	struct allocation {
 	...
 	struct success_failure {
 		large_t success;
 		large_t failure;
 		template<typename bool_> bool_ operator()(bool_ a) noexcept {
...
namespace _ { namespace statistic {
	...
	template<typename out_, size_t size_> void out(out_& o, char const (&n)[size_], allocation_success_failure const& a) {
		o << "  " << n << '\n'
	...
	template<typename out_, size_t size_> void out(out_& o, char const (&n)[size_], success_failure const& a) {
		o << "  " << n << '\n'
			<< "    success ............ " << a.success << '\n'
			<< "    failure ............ " << a.failure << '\n';
		}
...
template<typename out_> void out(out_& o, statistics const& a) {
