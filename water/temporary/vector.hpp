...
template<typename value_, typename allocator_, typename sizer_ = void> class
 vector {
	public:
		typedef value_
			value_type,
...
template<typename v_, typename a_, typename s_> void
 swap(vector<v_, a_, s_>& a, vector<v_, a_, s_>& b) noexcept {
	a.swap(b);
	}

template<typename v_, typename a_, typename s_> bool
 operator==(vector<v_, a_, s_> const& a, vector<v_, a_, s_> const& b) {
