...
namespace _ {
	template<typename out_> inline void
	 out_statistics_if(statistics& a)	{
	 	out_ o;
	 	out(o, a);
	 	}
	template<> inline void
	 out_statistics_if<void>(statistics&)
	 	{}
	}
...
template<typename allocator_ = void, typename sizer_ = void, typename out_ = void> class
 memory_track {
 	public:
...
template<typename allocator_, typename sizer_, typename out_> void
 swap(memory_track<allocator_, sizer_, out_>& a, memory_track<allocator_, sizer_, out_>& b) noexcept {
 	a.swap(b);
 	}
...
template<typename allocator_, typename sizer_, typename out_> allocator_throw<memory_track<allocator_, sizer_, out_> >
 allocator_for(memory_track<allocator_, sizer_, out_>& a) noexcept {
 	return allocator_throw<memory_track<allocator_, sizer_, out_> >{a};
 	}

template<typename allocator_, typename sizer_, typename out_> allocator_nothrow<memory_track<allocator_, sizer_, out_> >
 allocator_nothrow_for(memory_track<allocator_, sizer_, out_>& a) noexcept {
