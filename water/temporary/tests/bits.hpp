...
public:
    ...
    void free(void *pointer, size_t size) {
        ...
        dead(pointer, size + sizeof(cookie));
        --myallocations;
        my.free(pointer, size + sizeof(cookie));
    }
};
...
template<typename anything_>
not_a_str& operator<<(not_a_str& a, anything_ const&) { return a; }
...
template<int at_ = 0>
struct memory_types_do {
    template<typename function_>
    static void function(function_& function) {
...
template<>
struct memory_types_do<types::size<memory_types>::result> {
    template<typename function_>
    static void function(function_&) {}
};

template<typename function_>
void memory_types_function(function_&& function) {
