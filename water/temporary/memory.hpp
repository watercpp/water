...
template<typename allocator_ = void, typename sizer_ = void> class
 memory {
 	public:
 	...
 	public:
		...
		memory& operator=(memory&& a) noexcept {
 		...
 		memory& allocate_and_keep_blocks(unsigned a) const noexcept {
...
template<typename allocator_, typename sizer_> void
 swap(memory<allocator_, sizer_>& a, memory<allocator_, sizer_>& b) noexcept {
 	a.swap(b);
 	}
 	
template<typename allocator_, typename sizer_> allocator_throw<memory<allocator_, sizer_> >
 allocator_for(memory<allocator_, sizer_>& a) noexcept {
 	return allocator_throw<memory<allocator_, sizer_> >{a};
 	}

template<typename allocator_, typename sizer_> allocator_nothrow<memory<allocator_, sizer_> >
 allocator_nothrow_for(memory<allocator_, sizer_>& a) noexcept {
