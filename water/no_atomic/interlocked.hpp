...
template<typename type_> constexpr bool interlocked_is_atomic() {
...
template<typename int_> int_ interlocked_fetch_sub(int_ *a, int_ b, interlocked_order o) {
...
template<typename from_> struct
 cast<void*, from_ const*, false> {
 	static constexpr void* do_it(from_ a) {
		return const_cast<from_*>(a)
		}
 	};
template<typename to_, typename from_> struct
 cast<to_, from_, true> {
