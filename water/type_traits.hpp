...
namespace water {
    namespace _ {
        ...
        struct has_copy_constructor_do :
            types::bool_result<false>
        {};
        
        template<typename a_>
        struct has_copy_constructor_do<a_, types::to_void<decltype(a_(types::make<a_ const&>()))>> :
            types::bool_result<true>
        {};

        // visual c++ 2015 needed this extra indirection
        template<typename a_, typename = decltype(types::make<a_&>() = types::make<a_ const&>())>
        struct has_copy_assign_check {
            using result = void;
        };

        template<typename a_, typename = void>
        struct has_copy_assign_do :
            types::bool_result<false>
        {};
        
        template<typename a_>
        struct has_copy_assign_do<a_, typename has_copy_assign_check<a_>::result> :
            types::bool_result<true>
        {};
    }
