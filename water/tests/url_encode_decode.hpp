...
template<typename iterator_> void url_encode_decode(iterator_ begin, iterator_ end) {
...
template<size_t size_> void url_encode_decode(char const (&cstring)[size_]) {
	url_encode_decode(cstring + 0, cstring + size_ - 1);
	}

template<size_t size1_, size_t size2_> void url_encode_test(char const (&plain)[size1_], char const (&encoded)[size2_]) {
