...
#if !defined(WATER_ATOMIC_OLD_TESTS_TEST_HPP) || defined(WATER_ATOMIC_OLD_INLINE)
#ifndef WATER_ATOMIC_OLD_INLINE
#define WATER_ATOMIC_OLD_TESTS_TEST_HPP
 ...
 make0<alias<a_> > {
	...
	public:
		...
		operator alias<a_>() const {
			alias<a_> r;
			r.atomic() = water::atomic_old::raw_cast<typename alias<a_>::atomic_type>(static_cast<v_>(my_));
			return r;
			};
	};
...
template<typename a_> class
 make1<alias<a_> > {
	...
	public:
		...
		operator alias<a_>() const {
			alias<a_> r;
			r.atomic() = water::atomic_old::raw_cast<typename alias<a_>::atomic_type>(static_cast<v_>(my));
			return r;
			};
	};
...
template<typename a_, barrier_t b_> typename types::ifel_type<get_exists<a_, b_>, bool>::result get_test() {
	typedef typename water::atomic_old::value_type_of<a_>::result v_;
	make0<a_> m0;
	make1<a_> m1;
	a_ a = m0;
	v_ v = get<b_>(a);
