...
#ifndef WATER_ATOMIC_OLD_EXAMPLES_EXAMPLE1_HPP
#define WATER_ATOMIC_OLD_EXAMPLES_EXAMPLE1_HPP

/*

This file tries to document how to make an underlying implementation for water::atomic_old. it is written in plain c++, it is not atomic.

*/

#include <water/atomic_old/constants.hpp>
// the <water/atomic_old/constants.hpp> header contains
// - atomic_old::barrier_t and the none, acquire, release, full barriers
// - atomic_old::has_* flags

namespace water { namespace atomic_old { namespace example1 {
...
- template arguments:
  ...
  - barrier_t barrier_
- A compile-time constant has, an or-combination of water::atomic_old has flags
  - This tells water::atomic_old what functions exist
  - If there are no functions, has is 0

Atomic can have more template arguments, as long as they have default values. water::atomic_old will only
use the first 2

The functions are similar to the water::atomic_old functions except the first argument is a pointer
...
template<typename type_, water::atomic_old::barrier_t barrier_> struct
 atomic {
	enum has_ { has = 0 };
	};

template<water::atomic_old::barrier_t barrier_> struct
...
water::atomic_old will only use the first template argument, type_, any additional template arguments,
