...
#if !defined(WATER_ATOMIC_OLD_COMPARE_SET_ELSE_NON_ATOMIC_GET_HPP) || defined(WATER_ATOMIC_OLD_INLINE)
#ifndef WATER_ATOMIC_OLD_INLINE
#define WATER_ATOMIC_OLD_COMPARE_SET_ELSE_NON_ATOMIC_GET_HPP
...
template<barrier_t barrier_, typename type_> if_type<compare_set_else_non_atomic_get_exists<type_, barrier_>, bool>
 compare_set_else_non_atomic_get(type_& atomic, value<type_> compare, value<type_> set, value<type_>& get) {
	// This is exactly like atomic_old::compare_set_else_get, except when it is implemented using atomic_old::compare_set:
	//
	...
	//   return false;
	//
	// In some cases this could be more efficient than atomic_old::compare_set_else_get but use it only
	// if all possible bit-patterns of a value of type_ are valid.
	//
	// Example
	//
	//   void atomic_add1(atomic_old::int_t& a) {
	//       atomic_old::int_t b = a;
	//       while(!atomic_old::compare_set_else_non_atomic_get(a, b, b + 1, b));
	//       }
	//
