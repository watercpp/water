...
#ifndef WATER_ATOMIC_OLD_RAW_CAST_HPP
#define WATER_ATOMIC_OLD_RAW_CAST_HPP
...
template<typename to_, typename from_> to_
 raw_cast(from_ const& from) {
	// This is not an atomic operation. it is used by water::atomic_old to cast between the atomic_type
	// and the value_type of atomic_old::alias or other types with water::atomic_tag.
	//
	...
	//
	// This could maybe be useful to get/set the value of atomic_old::alias non-atomically
	//
	//   type t = ...;
	//   atomic_old::alias<type> a;
	//   a.atomic() = atomic_old::raw_cast<atomic_old::alias<type>::atomic_type>(t);
	//   t = atomic_old::raw_cast<type>(a.atomic());
	//
