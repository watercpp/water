...
#ifndef WATER_ATOMIC_OLD_ATOMIC_OPS_LOAD_HPP
#define WATER_ATOMIC_OLD_ATOMIC_OPS_LOAD_HPP
...
// AO_type_load sometimes return AO_t instead of type, static_cast to avoid warnings

#define WATER_ATOMIC_OLD_ATOMIC_OPS_make(type, prefix, barrier, if_) \
	template<typename i_> struct \
	 load<i_, barrier, type, typename if_not_ao_t<if_>::result> : has<has_get> { \
		static i_ do_it(i_ *a) { return static_cast<i_>(AO ## prefix ## load_ ## barrier(a)); } \
		};
#define WATER_ATOMIC_OLD_ATOMIC_OPS_make_none(type, prefix, if_) \
	template<typename i_> struct \
	 load<i_, none, type, typename if_not_ao_t<if_>::result> : has<has_get> { \
		static i_ do_it(i_ *a) { return static_cast<i_>(AO ## prefix ## load(a)); } \
		};
