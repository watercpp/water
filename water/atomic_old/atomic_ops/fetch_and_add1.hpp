...
#ifndef WATER_ATOMIC_OLD_ATOMIC_OPS_FETCH_AND_ADD1_HPP
#define WATER_ATOMIC_OLD_ATOMIC_OPS_FETCH_AND_ADD1_HPP
...
template<typename i_, barrier_t b_, typename x_ = i_, typename if_ = void> struct
 fetch_and_add1 : has<0> {
	static i_ do_it(i_*);
	};

#define WATER_ATOMIC_OLD_ATOMIC_OPS_make(type, prefix, barrier, if_) \
	template<typename i_> struct \
	 fetch_and_add1<i_, barrier, type, typename if_not_ao_t<if_>::result> : has<has_get_add1> { \
		static i_ do_it(i_ *a) { return AO ## prefix ## fetch_and_add1_ ## barrier(a); } \
		};
#define WATER_ATOMIC_OLD_ATOMIC_OPS_make_none(type, prefix, if_) \
	template<typename i_> struct \
	 fetch_and_add1<i_, none, type, typename if_not_ao_t<if_>::result> : has<has_get_add1> { \
		static i_ do_it(i_ *a) { return AO ## prefix ## fetch_and_add1(a); } \
		};
