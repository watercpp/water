...
template<typename o_> str::out<o_>& operator<<(str::out<o_>& o, str::specific<tag> a) {
	auto t = a.get();
	o << (t.name() ? t.name() : "unnamed");
	return o;
	}

template<typename o_, typename t_> str::out<o_>& operator<<(str::out<o_>& o, cookie<t_> c) {
	o << c.pointer << ' ' << c.bytes << " #" << c.id << ' ' << c.tag;
	return o;
	}

template<typename o_> void
 out(str::out<o_>& o, statistics const& s, unsigned indent = 0) {
...
template<typename o_, typename a_, typename t_> void
 out(str::out<o_>& o, memory<a_, t_>& m) {
