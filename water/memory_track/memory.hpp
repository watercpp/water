...
template<typename tag_ = void, typename callback_ = void> class
 memory {
	public:
		...
		static constexpr size_t
			align = align_max,
			overhead = sizeof(cookie_type) + pattern_size + pattern_size + ((sizeof(cookie_type) + pattern_size) % align ? align - ((sizeof(cookie_type) + pattern_size) % align) : 0);
	private:
	...
	private:
		cookie_error cookie_error_from(cookie_type const* c) const noexcept {
			...
			else if((upointer(c->next) % alignof(cookie_type)) || static_cast<void const*>(c->next) < lowest || static_cast<void const*>(c->next) > highest) {
				r.where = &c->next;
				r.error = "cookie next pointer";
				}
			else if((upointer(c->prev) % alignof(cookie_type)) || static_cast<void const*>(c->prev) < lowest || static_cast<void const*>(c->prev) > highest) {
				r.where = &c->next;
...
template<typename tag_, typename callback_> allocator_throw<memory<tag_, callback_> >
 allocator_for(memory<tag_, callback_>& a, typename memory<tag_, callback_>::tag_type const& t) noexcept {
 	return allocator_throw<memory<tag_, callback_> >{a, t};
 	}

template<typename tag_, typename callback_> allocator_throw<memory<tag_, callback_> >
 allocator_for(memory<tag_, callback_>& a) noexcept {
 	return allocator_throw<memory<tag_, callback_> >{a};
 	}

template<typename tag_, typename callback_> allocator_nothrow<memory<tag_, callback_> >
 allocator_nothrow_for(memory<tag_, callback_>& a, typename memory<tag_, callback_>::tag_type const& t) noexcept {
 	return allocator_nothrow<memory<tag_, callback_> >{a, t};
 	}

template<typename tag_, typename callback_> allocator_nothrow<memory<tag_, callback_> >
 allocator_nothrow_for(memory<tag_, callback_>& a) noexcept {
