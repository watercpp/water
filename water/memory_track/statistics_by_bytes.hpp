...
class statistics_by_bytes {
	public:
		...
		static size_t constexpr bytes_max_default = static_cast<size_t>(1024 * 1024 * 1024) ? static_cast<size_t>(1024 * 1024 * 1024) : static_cast<size_t>(-1);
	private:
	...
	private:
		template<typename iterator_> static iterator_ find_do(iterator_ b, iterator_ e, size_t a) noexcept {
			if(b == e)
				return 0;
			while(b->max < a) {
				auto i = b + (e - b) / 2;
