...
namespace _ { namespace no_numeric_limits {
	...
	template<
	 ...
	 bitsof_int :
		bitsof_int<int_, (value_ / (static_cast<int_>(1) << shift_)), result_ + shift_>
			{};
	template<typename int_, int_ value_, unsigned result_> struct
	 bitsof_int<int_, value_, result_, 0> :
		types::enum_result<result_>
			{};
	
	template<typename a_, int = !types::is_int<a_>::result ? 0 : types::is_unsigned<a_>::result ? 1 : -1> struct
	 max_min_int {
	...
	template<typename a_, bool = types::is_int<a_>::result> struct
	 digits : types::enum_result<
		...
		> {};
	template<> struct
	 digits<bool, types::is_int<bool>::result> : types::enum_result<1>
		{};
	template<typename a_> struct
	 digits<a_, true> : bitsof_int<a_, (max_min_int<a_>::max)()>
		{};
	 
	template<typename a_> struct
	 digits10 : types::enum_result<
	...
	template<typename a_> struct
	 max_exponent : types::enum_result<
	...
	template<typename a_> struct
	 min_exponent : types::enum_result<
	...
	template<typename a_> struct
	 max_exponent10 : types::enum_result<
	...
	template<typename a_> struct
	 min_exponent10 : types::enum_result<
	...
	template<typename a_> struct
	 max_min<a_, true> : max_min_int<a_> {
		static constexpr a_ lowest() noexcept { return (max_min_int<a_>::min)(); }
		};
	template<> struct
	 max_min<bool, types::is_int<bool>::result> {
	...
	template<typename float_, unsigned digits_ = digits<float_>::result, unsigned bits_ = bitsof_int<unsigned long long, static_cast<unsigned long long>(-1)>::result, bool = (digits_ <= bits_)> struct
	 denorm_min_divide {
	...
	template<typename float_, unsigned digits_, unsigned bits_> struct
	 denorm_min_divide<float_, digits_, bits_, false> {
	...
	template<typename a_> constexpr typename types::ifel_type<types::is_float<a_>, a_>::result     infinity() noexcept { return static_cast<a_>(INFINITY); }
	template<typename a_> constexpr typename types::ifel_type_not<types::is_float<a_>, a_>::result infinity() noexcept { return a_{}; }
	
	template<typename a_> constexpr typename types::ifel_type<types::is_float<a_>, a_>::result     nan() noexcept { return static_cast<a_>(NAN); }
	template<typename a_> constexpr typename types::ifel_type_not<types::is_float<a_>, a_>::result nan() noexcept { return a_{}; }

	// FLT_ROUNDS is not a compile time constant
	template<typename a_> constexpr typename types::ifel_type<types::is_float<a_>, a_>::result     round_error() noexcept { return static_cast<a_>(0.5); }
	template<typename a_> constexpr typename types::ifel_type_not<types::is_float<a_>, a_>::result round_error() noexcept { return a_{}; }
