...
namespace _ {
	template<typename a_, typename = void> struct
	 reverse_iterator_converts_to_bool :
		types::false_result
			{};
	template<typename a_> struct
	 reverse_iterator_converts_to_bool<a_, types::to_void<decltype(static_cast<bool>(types::make<a_ const&>()))>> :
	 	types::true_result
	 		{};
	}
...
template<typename iterator_> reverse_iterator<iterator_>
 reverse_iterator_from(iterator_ a) {
	return reverse_iterator<iterator_>(a);
	}

template<typename iterator_> reverse_iterator<iterator_>&
 operator+=(reverse_iterator<iterator_>& a, typename types::ifel_type<is_random_access_iterator<iterator_>, iterator_difference_type<iterator_>>::result b) {
	a.underlying() -= b;
	return a;
	}
...
template<typename iterator_> reverse_iterator<iterator_>
 operator+(typename types::ifel_type<is_random_access_iterator<iterator_>, iterator_difference_type<iterator_>>::result a, reverse_iterator<iterator_> b) {
	return b += a;
	}
...
template<typename iterator_> typename types::ifel_type<is_random_access_iterator<iterator_>, iterator_difference_type<iterator_>>::result
 operator-(reverse_iterator<iterator_> a, reverse_iterator<iterator_> b) {
	return b.underlying() - a.underlying();
	}

template<typename iterator_> bool
 operator<(reverse_iterator<iterator_> const& a, typename types::ifel_type<is_random_access_iterator<iterator_>, reverse_iterator<iterator_>>::result const& b) {
 	return b.underlying() < a.underlying();
 	}
