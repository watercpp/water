...
#ifndef WATER_TYPES_INTEGER_HPP
#define WATER_TYPES_INTEGER_HPP
...
- result_tag is a typedef of types::result_tag<types::integer_kind>

struct zero {
	enum type { result = 0 };
	typedef types::result_tag<types::integer_kind> result_tag;
	};
...
A special property of types::integer is that it enables nesting using types::int_if_integer. This is
not very useful for types::integer by itself, but it can be useful for creating templates that accept
a single type argument that is an integer result, or two arguments type + itegral constant:

	template<
 		typename type_or_result_,
 		typename types::int_if_integer<type_or_result_>::result result_ = 0
 		> struct
 	 div2 :
		types::integer<
			typename types::integer<type_or_result_, result_>::type,
			types::integer<type_or_result_, result_>::result / 2
			> {};
...
struct integer_kind {
	template<typename a_, typename b_> struct
	 equal : bool_result<a_::result == b_::result>
		{};
	template<typename a_> struct
	 type : type_plain<typename a_::type>
		{};
	template<typename a_> struct
	 to_bool : bool_result<a_::result ? true : false>
		{};
	};
	...
	template<
	 ...
	 do_int_if_number :
		type_plain<a_>
			{};
	template<typename a_> struct
	 do_int_if_number<a_, typename a_::result_tag, void, true> :
		type_plain<int>
			{};
	template<typename a_> struct
	 do_int_if_number<a_, typename a_::result_tag, void, false> :
		do_int_if_number<a_, void, typename a_::result_tag, false>
			{};
	template<typename a_> struct
	 do_int_if_number<a_, void, result_tag<integer_kind>, false> :
		type_plain<int>
			{};
	}

// if the types::type<type_>::result is a types::integer result int, else result types::type<type_>
...
namespace _ {
	template<typename t_, typename i_, i_ r_> struct
	 do_integer :
		t_
			{};
	template<typename t_, t_ r_> struct
	 do_integer<t_, t_, r_> {
...
template<typename type_or_result_, typename int_if_integer<type_or_result_>::result result_ = 0> struct
 integer :
	_::do_integer<
