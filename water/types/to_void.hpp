...
struct has_some_type<type_, types::to_void<typename type_::some_type>>  :
    types::true_result
{};
...
template<typename type_>
struct has_begin_end<type_, types::to_void<decltype(types::make<type_&>().begin() != types::make<type_&>().end())>>  :
    types::true_result
{};
