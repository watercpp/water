...
template<typename char_, typename memory_> class
 utf_convert_in_place {
 	...
 	static unsigned constexpr utf = unicode::utf_from_char<char_>::result;
 	template<unsigned> struct select {};
	private:
		memory_ *mymemory;
		void *mypointer;
		encoding myencoding;
	...
	private:
		template<unsigned from_utf_, typename iterator_> void convert(unicode::utf_length<from_utf_> length, iterator_ from, select<0>) {
			// copy
			size_t
		...
		template<typename iterator_> void endian_change_maybe(iterator_ from) {
			char_ const one = 1;
