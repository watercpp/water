...
#include <water/xml/utf_convert_in_place.hpp>
...
public:
    ...
    read& parse_in_place(void *pointer, size_t bytes, callback_&& callback) {
        ...
        my = 0;
        utf_convert_in_place<char_type, memory_type> convert(mymemory, pointer, bytes);
        parse_in_place(convert.begin(), convert.end(), callback);
        return *this;
    }
...
read<char_, memory<allocator_>> read_to_memory(memory<allocator_>& a) {
    return {a};
}

template<typename allocator_>
read<char, memory<allocator_>> read_to_memory(memory<allocator_>& a) {
    return {a};
}
