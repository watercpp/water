...
template<typename memory_> node<char16_t, memory_> unicode_generate(memory_& m) {
...
class unicode_test {
	...
	private:
		template<typename to_, typename from_> void parse2(vector<from_>* from, node<char16_t> original) {
			mymemory.clear();
		...
		template<typename char_> void write(vector<unsigned char>& to, node<char_, memory<>> from, bool indent) {
		...
		template<typename char_> void write(vector<char_>& to, node<char_, memory<>> from, bool indent) {
			size_t size = write_size(from, indent ? true : false, indent ? '\t' : 0);
			___water_test(size);
			to.clear();
			to.reserve(size);
