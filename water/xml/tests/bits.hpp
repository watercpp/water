...
template<typename a_, typename b_> bool equal_ranges(a_ const& a, b_ const& b) {
	return equal_ranges(a.begin(), a.end(), b.begin(), b.end());
	}

template<typename left_, typename right_> struct
 equal_return {
	left_ left;
	right_ right;
...
template<typename cl_, typename ml_, typename cr_, typename mr_> equal_return<node<cl_, ml_>, node<cr_, mr_>>
 equal(node<cl_, ml_> l, node<cr_, mr_> r) {
