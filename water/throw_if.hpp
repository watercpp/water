...
// do not depend on anything in water
	...
	namespace water {
		...
		template<typename exception_> inline constexpr bool
		 throw_if_noexcept() {
		 	return false;
		 	}
		template<> inline constexpr bool
		 throw_if_noexcept<void>() {
		 	return true;
		 	}
	
		template<typename exception_> inline void
		 throw_if() noexcept(throw_if_noexcept<exception_>()) {
			throw exception_();
			}
		template<> inline void
		 throw_if<void>() noexcept(throw_if_noexcept<void>())
			{}
	}
