...
template<typename iterator_, bool make_const_ = false, bool proxy_ = false> class
 bidirectional {
	static_assert(types::type_assert<is_bidirectional_iterator<iterator_>>::assert, "must be at least bidirectional");
...
template<typename iterator_> bidirectional<iterator_> bidirectional_from(iterator_ const& a) {
	return bidirectional<iterator_>{a};
	}

template<typename iterator_> bidirectional_const<iterator_> bidirectional_const_from(iterator_ const& a) {
	return bidirectional_const<iterator_>{a};
	}

template<typename iterator_> bidirectional_proxied<iterator_> bidirectional_proxied_from(iterator_ const& a) {
	return bidirectional_proxied<iterator_>{a};
	}

template<typename iterator_> bidirectional_proxied_const<iterator_> bidirectional_proxied_const_from(iterator_ const& a) {
