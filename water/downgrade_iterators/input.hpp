...
template<typename iterator_, bool proxy_ = false> class
 input {
...
template<typename iterator_> input<iterator_> input_from(iterator_ const& a) {
	return input<iterator_>{a};
	}

template<typename iterator_> input_proxied<iterator_> input_proxied_from(iterator_ const& a) {
