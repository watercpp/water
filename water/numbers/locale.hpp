...
struct locale {
	begin_end<char const*> digits() const {
		return {u8"0123456789ABCDEF", 16};
		}
	begin_end<char const*> pluses() const {
		return {u8"+", 1};
		}
	...
	begin_end<char const*> points() const {
		return {u8".,", 2};
		}
	...
	begin_end<locale_const_string<char> const*> nans() const {
	...
	begin_end<locale_const_string<char> const*> trues() const {
		return {_::locales::trues, _::locales::trues +  sizeof(_::locales::trues) / sizeof(_::locales::trues[0])};
		}
	begin_end<locale_const_string<char> const*> falses() const {
