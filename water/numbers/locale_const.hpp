...
template<typename char_, typename group_ = group>
struct locale_const
{
    locale_const_string<char_>
        mydigits,
...
private:
    ...
    size_t size(type_ const (&a)[size_]) const {
        size_t r = 0;
        while(a[r] && ++r != size_);
        return r;
    }
