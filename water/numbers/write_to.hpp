...
namespace _ {
    ...
    struct write_one
    {
        ...
        static unsigned do_it(to_& to, char32_t from) {
    ...
    struct write_one<8>
    {
        ...
        static unsigned do_it(write_nothing& /*to*/, char_ from) {
            if(from <= 0x7f)
                return 1;
            if(from <= 0x7ff)
                return 2;
    ...
    struct write_one<16>
    {
        ...
        template<typename to_>
        static unsigned do_it(to_& to, char16_t from) {
...
template<typename iterator_, typename = void>
class write_to_begin_end
{
    iterator_ my, myend;
