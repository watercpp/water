...
namespace _ {
	...
	template<
	 ...
	 hibit :
		hibit<(a_ >> x_), r_ + x_>
			{};
	template<unsigned long r_, unsigned long x_> struct
	 hibit<0, r_, x_> :
	...
	template<unsigned long a_> struct
	 log2_lookup :
		types::integer<
			unsigned long,
			a_ == 1 ? 2909ul :
	...
	template<
	 ...
	 max_binary
		{};
	template<unsigned r_, unsigned d_> struct
	 max_binary<bool, r_, d_, 'i'> :
		types::integer<unsigned, 1>
			{};
	template<typename a_, unsigned r_, unsigned d_> struct
	 max_binary<a_, r_, d_, 'i'> :
		types::integer<unsigned, numeric_limits<a_>::digits + (numeric_limits<a_>::is_signed && is_twos_complement<a_>() ? 1 : 0)>
			{};
	template<typename a_, unsigned r_, unsigned d_> struct
	 max_binary<a_, r_, d_, '2'> :
		types::integer<
			unsigned,
	...
	template<typename a_, bool i_ = types::is_int<a_>::result> struct
	 max_decimal :
		types::integer<
			unsigned,
	...
	template<typename a_> struct
	 max_decimal<a_, true> : // log10(2) * (1 << 16) = 19728
		types::integer<
			unsigned,
...
template<typename type_> constexpr unsigned max_binary_digits() {
...
template<typename type_> constexpr unsigned max_decimal_digits() {
...
namespace _ {
	template<unsigned r_, typename t_, bool pow2_ = (r_ & (r_ - 1)) == 0> struct
	 max_digits_do {
	...
	template<unsigned r_, typename t_> struct
	 max_digits_do<r_, t_, false> {
		static unsigned do_it(unsigned base) {
...
template<typename type_> unsigned max_digits(unsigned base = 10) {
...
template<typename type_> unsigned max_fraction_digits(unsigned base = 10) {
	// return
