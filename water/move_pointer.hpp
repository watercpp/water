...
template<typename type_, typename allocator_> class
 move_pointer {
	public:
...
template<typename type_, typename allocator_> void
 swap(move_pointer<type_, allocator_>& a, move_pointer<type_, allocator_>& b) {
	a.swap(b);
	}

template<typename type_, typename allocator_> move_pointer<type_, allocator_>
 move_pointer_from(type_ *pointer, allocator_ const& allocator) {
	return move_pointer<type_, allocator_>{pointer, allocator};
	}

}

template<typename type_, typename allocator_> void* operator new(decltype(sizeof(0)), water::move_pointer<type_, allocator_>& a) {
	return a.operator_new();
	}

template<typename type_, typename allocator_> void operator delete(void*, water::move_pointer<type_, allocator_>& a) {
	a.operator_delete();
	}
